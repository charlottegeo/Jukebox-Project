...
document.addEventListener('DOMContentLoaded', function() {
            ...
            case 'next_song':
                ...
                if (data.nextSong) {
                    if (data.nextSong.source === 'spotify') {
                        ...
                        playMp3File(data.nextSong.mp3_url); // Stream from S3
                    }
    ...
    socket.on('mp3ReadyForPlayback', function(data) {
        const mp3Url = data.mp3_url;
        console.log('MP3 file is ready for playback:', mp3Url);
        
        // Ensure that the MP3 file starts playing only after data is fully loaded
        const trackData = data.track_data;
        if (trackData && mp3Url) {
            // Set the MP3 file URL to the audio player and play it
            playMp3File(mp3Url);
            playSong(trackData); // Update the UI with the current song data
        } else {
            console.error('Failed to load MP3 or track data is missing.');
        }
    });
    if (window.location.pathname == "/") {
...
function playMp3File(mp3Url) {
    const mp3Player = document.getElementById('mp3-player');

    if (!mp3Url) {
        console.error('MP3 file URL is not defined or invalid:', mp3Url);
        return;
    }

    console.log('Playing MP3 file:', mp3Url);
    mp3Player.src = mp3Url;

    mp3Player.onerror = function() {
        console.error('Error loading MP3 file:', mp3Url);
        mp3Player.src = '';
        return;
    };

    mp3Player.onloadeddata = function() {
        console.log('MP3 file is fully loaded, starting playback.');
...
function playSong(song = null) {
    ...
    const mp3PlayerWrapper = document.getElementById('mp3-player-wrapper');

    if (song.source === 'spotify') {
        if (mp3PlayerWrapper) mp3PlayerWrapper.style.display = 'none';
        ...
        if (mp3PlayerWrapper) mp3PlayerWrapper.style.display = 'block';

        playMp3File(song.mp3_url);
...
function updatePlayerProgress(trackLength) {
    ...
    if (mp3Player && mp3Player.currentTime && mp3Player.duration) {
        var progress = (mp3Player.currentTime / mp3Player.duration) * 100;
        document.getElementById('progressBar').value = progress;
        document.getElementById('duration').innerHTML = formatTime(mp3Player.duration);
        document.getElementById('progressTimestamp').innerHTML = formatTime(mp3Player.currentTime);
    } else {
